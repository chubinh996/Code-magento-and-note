 ...
 */
class Rycoweb_SpecialPromotions_Model_Validator extends Mage_SalesRule_Model_Validator
{
    /**
        ...
        foreach ($this->_getRules() as $rule) {
            ...
            switch ($rule->getSimpleAction()) {
                ...
                case Mage_SalesRule_Model_Rule::BY_PERCENT_ACTION:
                    ...
                    $itemBulkPrice = $this->_getItemBulkPrice($item);
                    $step = $rule->getDiscountStep();
                    if ($step) {
                        $qty = floor($qty/$step)*$step;
                    }

                    if($rule->getCouponType() != Mage_SalesRule_Model_Rule::COUPON_TYPE_NO_COUPON && $rule->getCouponCode() != "") {
                ...
                case Rycoweb_SpecialPromotions_Model_Rule::USE_BULK_PRICE_ACTION:
                    $itemBulkPrice = $this->_getItemBulkPrice($item);
                    $useBulkPriceFlag = true;
                    break;
                //EOF: new case condition for new rule USE_BULK_PRICE_ACTION
            }
     ...
     * @return float
     */
    protected function _getItemBulkPrice($item)
    {
        return Mage::helper('tax')->getPrice($item, $item->getBulkPrice(), true);
    }
    public function getRulesDiscount(){
