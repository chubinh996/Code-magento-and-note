...
class Rycoweb_ImageGallery_Adminhtml_Imagegallery_ImagegalleryController extends Mage_Adminhtml_Controller_Action {
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            ...
            $landscapeimage = $model->setImage('landscapeimage');
            //$portraitimage = $model->setImage('portraitimage');

            if (!$id && !$landscapeimage) {
           ...
           /* if ($portraitimage) {
            ...
            if ($landscapeimage) {
                $data['landscapeimage'] = $landscapeimage;
            }
    ...
    public function deleteAction() {
        if ($this->getRequest()->getParam('id') > 0) {
            try {
                ...
                if (file_exists(Mage::getBaseDir('media') . DS . $model->getPortraitimage())) {
                    unlink(Mage::getBaseDir('media') . DS . $model->getPortraitimage());
                }
                if (file_exists(Mage::getBaseDir('media') . DS . $model->getLandscapeimage())) {
                    unlink(Mage::getBaseDir('media') . DS . $model->getLandscapeimage());
                }
