...
class MassReject extends \Aheadworks\Pquestion\Controller\Adminhtml\Question
{
    /**
    ...
    public function execute()
    {
        ...
        foreach ($this->_collection->getItems() as $question) {
            if ($question->getStatus() == \Aheadworks\Pquestion\Model\Source\Question\Status::DECLINE_VALUE) {
                continue;
            }
            $question
                ->setStatus(\Aheadworks\Pquestion\Model\Source\Question\Status::DECLINE_VALUE)
                ->save()
            ;
            $count++;
        }
        $this->messageManager->addSuccess(
            __('A total of %1 record(s) have been rejected.', $count)
        );
